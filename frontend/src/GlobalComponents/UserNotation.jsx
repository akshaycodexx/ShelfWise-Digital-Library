...
const UserNotation = ({ name, img }) => {
  const { setUserToken,setLoggedInUserData  } = useContext(AuthContext);
  ...
  return (
    <div className="min-w-[200px] flex items-center bg-[#c17130] rounded-md py-1 px-2 my-2 justify-center gap-5 border border-[#c17130] relative">
      <div className="h-[60px] w-[60px] rounded-full">
        <img className="h-full w-full rounded-full object-cover" src={img} alt="" />
      </div>
        ...
        {toggleArrow ? (
          <>
            ...
            <div className="absolute h-[100%] flex items-center justify-center w-fit p-2 bg-[#ffff] rounded-md top-0 z-10 -right-16">
              <img
                src={assets.logout}
                className="h-[40px] w-[40px] m-auto bg-[white] rounded-full cursor-pointer"
                onClick={handleUserLogout} // ✅ Click on image directly logs out
                alt="Logout"
              />
            </div>
          </>
